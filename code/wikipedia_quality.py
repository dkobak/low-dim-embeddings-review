...
L_X_LD = list()
L_names = [(paths.pca_name, paths.pca_path), (paths.mds_name, paths.mds_path), (paths.LE_name_no_param, paths.LE_paths_no_param), (paths.tsne_name_no_param, paths.tsne_path_no_param), (paths.umap_name_no_param, paths.umap_path_no_param), (paths.phate_name_no_param, '{v}_30nn'.format(v=paths.phate_path_no_param))]
for name, npath in L_names:
    L_X_LD.append(np.load('{ep}{v}'.format(ep=paths.wiki_emb, v="wikipedia_{npath}_data_map.npy".format(npath=npath))))
print("===")
n_embs = len(L_X_LD)
